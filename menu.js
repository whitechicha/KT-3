...
const createUl = (_list, parent) => {
    ...
    if (Array.isArray(_list)) {
        for(let i = 0; i < _list.length; i++){
            ...
            li.innerHTML = _list[i].name && !withChildren
...
function clickUlHandler(e) {
    ...
    if (e.traget.open) lastElementChild.classList.add("hidden");
    else lastElementChild.classList.remove("hidden");

    const open_icon = e.target.getAttribute("icon-opened") === "-" ? "+" : "-";
